...
type Agent = { id: string; code: string; name: string; description: string; prompt: string; type: 'codegen' | 'other'; skillGroupId?: string; skillGroup?: { name: string }; mcpNames: string[] };
...
export default function AgentsPage() {
  ...
  return <AdminShell>
    ...
    <Table rowKey="id" dataSource={agents} columns={[
      { title: '名称', dataIndex: 'name', render: (name: string, row: Agent) => <Link href={`/conversations?agentCode=${encodeURIComponent(row.code)}`}><Space><RobotOutlined />{name}</Space></Link> },
      { title: 'Code', dataIndex: 'code', render: (code: string) => <code>{code}</code> },
    ...
    <Drawer open={open} width={720} title={editing ? '编辑智能体' : '新增智能体'} destroyOnClose onClose={() => setOpen(false)} extra={<Button type="primary" onClick={() => form.submit()}>保存</Button>}>
      <Form form={form} layout="vertical" onFinish={submit} className="pt-4">
        ...
        <Form.Item name="code" label="Code" rules={[{ pattern: /^[a-z0-9]+(?:-[a-z0-9]+)*$/, message: '只能使用小写字母、数字和短横线' }]}><Input placeholder="例如 frontend-builder" /></Form.Item>
        ...
        <Form.Item name="prompt" label="提示词"><RichTextEditor minHeight={320} placeholder="输入智能体的角色、边界和工作要求" /></Form.Item>
